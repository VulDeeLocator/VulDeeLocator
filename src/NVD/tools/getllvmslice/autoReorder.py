...
def reorder(targetPath, fileName, flag):
    if not os.path.exists(targetPath):
        return
    flawLineDict = {} #key:xxx_new.ll value
    focusLineDict = {} #key:xxx_new.ll value
    dirList = os.listdir(targetPath) 
    flagZero = 0
    linenum = 0
    for targetfile in dirList:  #file in four kinds of focus
    	flagZero = 0
        ...
        if flagZero == 0 and sliceName in flawLineDict:
            for i in range(int(targetflag[0][1])): #add vulnerable line
