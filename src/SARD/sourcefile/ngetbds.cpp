...
void getCursorSpelling2( CXCursor cursor ) {
	...
	auto fileName = clang_getFileName(file); //因为我们没有实现跨文件，单一源文件，所以content是本文件名
...
CXChildVisitResult functionVisitor(CXCursor cursor, CXCursor parent, CXClientData clientData) {
	...
	if(kind == CXCursorKind::CXCursor_DeclRefExpr && tag == string::npos) { //对结构体指针进行判断
		string key1 = "[";
		string key2 = "*";
...
CXChildVisitResult findchild(CXCursor cursor, CXCursor parent, CXClientData clientData) {
	...
	string defname = clang_getCString(defcursorspelling);	//真实类型
	...
	if(defname != "" && result!="") { //typedef定义过
		map<string, string>::iterator iter;
		iter = typmap.find(result);
		if(iter == typmap.end()) { //未加入map
	...
	// += -=
		...
		getCursorSpelling2(cursor);	//输出行号
		...
		for (unsigned int i = 0; i < nTokens; i++)
		{
			...
			if(kind == CXToken_Punctuation){
				if(strcmp(clang_getCString(s), "=") == 0){	//第一个符号是 "="
					if(clang_visitChildren(cursor, varjudge, nullptr)){ //判断有一个是变量
						getCursorSpelling2(parent);	//输出行号
